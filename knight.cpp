...
Knight::Knight(Color color, const QPoint& startLocation, const QPixmap& pixmap, QGraphicsItem* parent)
...
QList<QPoint> Knight::calculateValidMoves(int startX, int startY) const {
    ...
    const QList<QPoint> knightMoves = {
        QPoint(startX + 2, startY + 1), QPoint(startX + 2, startY - 1),
        QPoint(startX - 2, startY + 1), QPoint(startX - 2, startY - 1),
        QPoint(startX + 1, startY + 2), QPoint(startX + 1, startY - 2),
        QPoint(startX - 1, startY + 2), QPoint(startX - 1, startY - 2)
    };
